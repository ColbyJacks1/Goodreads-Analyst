...
GEMINI_RECOMMENDATION_MODEL=gemini-3-pro-preview (optional)
```

**Important:** The app supports both local `.env` files and cloud environment variables:
- **Local Development**: Create a `.env` file (gitignored) with your API keys
- **Streamlit Cloud**: Set environment variables in the dashboard (see below)
- The code automatically uses whichever is available - cloud env vars take precedence
...
4. **Set environment variables in the dashboard:**
   - Click "Advanced settings" or "Secrets"
   - Add: `GOOGLE_GEMINI_API_KEY` = `your_actual_api_key_here`
   - Optionally add: `GEMINI_RECOMMENDATION_MODEL` = `gemini-3-pro-preview`
   - **Note:** These are set in the Streamlit Cloud UI, NOT in a `.env` file
5. Deploy!

**Why not commit `.env`?**
- `.env` files contain sensitive API keys that should never be in Git
- Streamlit Cloud provides a secure way to set environment variables in their dashboard
- Your app code reads from environment variables, so it works the same way

...
def show_comprehensive_analysis_page_parallel():
    ...
    if not api_key or api_key == "your_openai_api_key_here" or api_key.startswith("your_"):
        st.error("❌ **API Key Not Configured**")
        st.warning("""
        **To enable AI analysis, you need to set up your Google Gemini API key:**
        
        1. **Local Development**: Create a `.env` file in the project root with:
           ```
           GOOGLE_GEMINI_API_KEY=your_actual_api_key_here
           ```
        
        2. **Get your API key**: Visit [Google AI Studio](https://makersuite.google.com/app/apikey) to get a free API key
    ...
    if not can_generate or (can_generate and st.session_state.get('analysis_status', 'not_started') == "not_started"):
        ...
        if can_generate:
            ...
            if st.button("🔮 Start Analysis", type="primary", use_container_width=True):
                ...
                # Clear ALL previous analysis state to force fresh generation
                st.session_state.pop('comprehensive_analysis_result', None)
                st.session_state.pop('comprehensive_analysis_sections', None)
                st.session_state.pop('comprehensive_analysis_sections_parallel', None)
                st.session_state.pop('quick_analysis_sections', None)
                st.session_state.pop('quick_analysis_result', None)
                st.session_state.pop('quick_analysis_completed', None)
                ...
                with st.spinner("Generating quick analysis (roast + recommendations)..."):
                    try:
                        ...
                        if quick_result.get("success"):
                            ...
                            error_msg = quick_result.get("error", "Unknown error")
                            st.session_state.analysis_error = error_msg
                            # Show helpful error message
                            if "GOOGLE_GEMINI_API_KEY" in error_msg:
                                st.error(f"❌ **API Key Missing**: {error_msg}")
                                st.info("💡 **To fix this:**\n1. Make sure your `.env` file contains `GOOGLE_GEMINI_API_KEY=your_key_here`\n2. Restart the Streamlit app after adding the key")
                            else:
                ...
                if 'comprehensive_analysis_started' not in st.session_state:
                    ...
                    try:
                        ...
                        if comprehensive_result.get("success"):
                            ...
                            st.warning(f"⚠️ Comprehensive analysis failed: {error_msg}")
                            # Still show quick analysis results if available
                            if st.session_state.get("quick_analysis_sections"):
